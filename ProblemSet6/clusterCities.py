...
class Cluster(object):
    ...
    def averageLinkageDist(self, other):
        ...
        for point in self.points:            
            for p in other.points:
                s+=point.distance(p)
        mean=s/(len(self.points)*len(other.points)) 
        return mean       
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        c = Cluster((c1.points + c2.points), c2.pointType)
    ...
    def mergeOne(self, linkage):
        ...
        tu=ClusterSet.findClosest(self,linkage)
        ClusterSet.mergeClusters(self, tu[0], tu[1])        
        return tu
...
def test():
    points = buildCityPoints('F:/notes/advanced/W6/ProblemSet6/cityTemps.txt', False)
    hCluster(points, Cluster.singleLinkageDist, 10, False)
    points = buildCityPoints('F:/notes/advanced/W6/ProblemSet6/cityTemps.txt', True)
